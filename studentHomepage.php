<?php
	...
	session_start();
			$sql = " SELECT distinct t.id, t.topic, sc.student_id, c.course_code, if(isnull(tia.id), 'no', 'yes') as is_taken
						FROM `topic` t 
 			...
 			$sqlTopic = "select distinct c.* from courses c
						 inner join student_classes sc on sc.course_id = c.id
						 where sc.student_id = " . $_SESSION['id'];
 			$resultTopic = mysqli_query($con, $sqlTopic);



			$sqlAnswer  = " SELECT distinct ti.id, tia.student_id, ti.course_id, ti.topic_id, ti.question, ti.boolean_answer,ti.single_answer,
